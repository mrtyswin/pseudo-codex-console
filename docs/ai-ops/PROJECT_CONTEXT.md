...
2. **添付画像が ChatGPT に届いていない疑い。** ジョブに添付したスクショがコンソール側ディスクには
   保存されるが、デーモンの `ask` ログが `upload=none` で、ChatGPT に画像がアップロードされず、
   モデルが画像を見ずに位置を推測して外す事例がある（2026-07-19 のモバイル余白ジョブ）。
   画像必須の依頼では、`upload=` が none でないかを疑うこと。（**未解決**、`DECISIONS.md`/未完了事項参照。）
...
- 添付画像が ChatGPT にアップロードされない問題（地雷 2）。送信側の欠陥、**未着手**。
