...
const workspace = workspaceInput && !path.isAbsolute(workspaceInput)
...
'<div class="config-section-head"><h3>通常はこの2項目だけ入力</h3><p>プロジェクト名と、コードが置かれているフォルダを指定します。保存しても作業は始まらず、「新しいジョブ」の実行先に追加されます。</p></div>',
'<div class="project-config-grid">',
'<div><label for="project-config-name">プロジェクト名 <span class="field-required">必須</span></label><input id="project-config-name" name="name" maxlength="64" placeholder="wordpress-demo" required><small class="field-help">新規ジョブの選択肢に表示する名前です。Ubuntuのフォルダ名と同じでなくても構いません。</small></div>',
'<div><label for="project-config-workspace">コードが置かれているフォルダ <span class="field-required">必須</span></label><input id="project-config-workspace" name="workspace" maxlength="500" placeholder="/home/ubuntu/chatgpt-projects/wordpress-demo" required><small class="field-help">package.json、compose.yaml、wp-contentなどがあるプロジェクト直下の絶対パスを入力します。親フォルダや本番の配置先ではありません。</small></div>',
'</div>',
'<p class="helper"><strong>ここまでで登録できます。</strong> 通常設定ではUbuntu上の専用作業フォルダで変更とテストを行い、本番への自動反映はしません。</p>',
'</section>',
