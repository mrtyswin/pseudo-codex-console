...
'<div class="config-section-head"><h3>1. 実行先プロジェクト</h3><p>ここで追加するのはジョブではなく、ジョブ実行時に選ぶ「プロジェクト設定」です。新しい名前で保存すると追加、登録済みと同じ名前で保存すると更新します。</p></div>',
...
'<div><label for="project-config-workspace">Ubuntu Workspace Path <span class="field-required">必須</span></label><input id="project-config-workspace" name="workspace" maxlength="500" placeholder="/home/ubuntu/chatgpt-projects/wordpress-demo" required><small class="field-help">対象コードが実際に存在するUbuntu上の絶対パスです。推測で入力せず、Ubuntuで確認したパスをそのまま指定します。</small></div>',
'</div>',
