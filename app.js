...
'</section>',
'<section class="config-section">',
'<div class="config-section-head"><h3>2. 本番反映・動作確認</h3><p>すでにUbuntu上で動いているWebアプリやサービスへ、自動デプロイする場合だけ設定します。反映先や実行コマンドが決まっていない場合は空欄のままにします。</p></div>',
'<div class="project-config-grid">',
'<div><label for="project-config-health">稼働確認URL <span class="field-optional">任意</span></label><input id="project-config-health" name="healthUrl" maxlength="500" placeholder="http://127.0.0.1:8080/health"><small class="field-help">デプロイ後にアクセスして正常応答を確認できるURLです。専用URLがなければ空欄にします。</small></div>',
'<div><label for="project-config-production">本番配置先パス <span class="field-optional">任意</span></label><input id="project-config-production" name="productionRoot" maxlength="500" placeholder="/opt/pseudo-codex-console"><small class="field-help">現在本番で動作しているファイルのUbuntu上の絶対パスです。</small></div>',
'<div><label for="project-config-service">サービス・コンテナ名 <span class="field-optional">任意</span></label><input id="project-config-service" name="service" maxlength="300" placeholder="pseudo-codex-console.service"><small class="field-help">systemdサービス名やDockerコンテナ名など、再起動・状態確認に使う既存の名前です。</small></div>',
'<div><label for="project-config-deploy">本番反映コマンド <span class="field-optional">任意</span></label><input id="project-config-deploy" name="deployCommand" maxlength="500" placeholder="/usr/local/libexec/pseudo-codex-deploy-request-console"><small class="field-help">検証成功後にUbuntuで実行する、既存のデプロイスクリプトまたはコマンドです。</small></div>',
'<div><label for="project-config-verify">反映確認コマンド <span class="field-optional">任意</span></label><input id="project-config-verify" name="verifyCommand" maxlength="500" placeholder="/usr/local/lib/pseudo-codex-console-deploy/verify-live.js"><small class="field-help">デプロイ後、本番へ正しく反映されたことを確認する既存のコマンドです。</small></div>',
'<div><label for="project-config-owner">デプロイ実行担当 <span class="field-optional">任意</span></label><input id="project-config-owner" name="deploymentOwner" maxlength="100" placeholder="host_dispatcher"><small class="field-help">通常はhost_dispatcherを指定します。独自の実行担当がある場合だけ変更します。</small></div>',
'</div>',
'<div class="checkbox-row"><label><input type="checkbox" name="requiresDeployment" value="true"> 上記のコマンドを使い、検証成功後に本番反映する</label></div>',
'<p class="helper">入力するのは新しく考えた値ではなく、そのプロジェクトですでに使用している本番パス・サービス名・デプロイ手順です。</p>',
'</section>',
'<section class="config-section">',
