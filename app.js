...
function updateQueueSummary(jobs) {
  ...
  var completed = jobs.filter(function (job) { return job.stage === 'completed'; }).length;
  ...
  node.innerHTML = '<span><strong>実行中</strong> ' + running + '</span>' +
    '<span><strong>待機中</strong> ' + queued + '</span>' +
    '<span><strong>完了</strong> ' + completed + '</span>' +
    '<span><strong>失敗</strong> ' + failed + '</span>' +
    '<span><strong>停止</strong> ' + stopped + '</span>';
}
...
durationText: formatJobDuration(job),
...
const workspaceInput = String(value.workspace || "").trim();
const workspaceMode = value.workspaceMode === "new"
? "new"
: value.workspaceMode === "existing"
? "existing"
: workspaceInput
? "existing"
: "new";
const workspace = workspaceMode === "new"
? path.join("/home/ubuntu/chatgpt-projects", name)
: workspaceInput && !path.isAbsolute(workspaceInput)
? path.join("/home/ubuntu", workspaceInput)
: workspaceInput;
...
throw new Error("既存プロジェクトを使う場合は、コードがあるフォルダを入力してください");
}
...
name,
config,
createWorkspace: workspaceMode === "new"
};
}
...
phase: "INSPECT",
remainingTurns: 0,
strategyPass: (job.strategyPass || 1) + 1,
inspectTurns: 0,
...
if (entry && entry.stage && entry.stage !== "queued" && entry.at) {
return entry.at;
}
}
return job.stage && job.stage !== "queued" ? job.createdAt : null;
}

function formatJobDuration(job, now) {
const startedAtValue = jobStartedAt(job);
...
const totalMinutes = Math.max(0, Math.floor((endAt - startedAt) / 60000));
...
'<span><strong>実行中</strong> ', String(counts.running), '</span>',
'<span><strong>待機中</strong> ', String(counts.queued), '</span>',
'<span><strong>完了</strong> ', String(counts.completed), '</span>',
'<span><strong>失敗</strong> ', String(counts.failed), '</span>',
'<span><strong>停止</strong> ', String(counts.stopped), '</span>',
'</section>'
].join("");
}
...
'<div class="config-section-head"><h3>最初にプロジェクトの状態を選択</h3><p>新しく作る場合は、フォルダを用意したり保存場所を決めたりする必要はありません。既存コードを使う場合だけ、そのフォルダを指定します。</p></div>',
'<div class="execution-mode-guide" aria-label="プロジェクトの状態">',
'<div><strong>これから新しく作る</strong>プロジェクト名だけ入力します。作業フォルダは自動で決定して作成します。</div>',
'<div><strong>すでにあるコードを使う</strong>package.json、compose.yaml、wp-contentなどがあるプロジェクト直下のフォルダを指定します。</div>',
'</div>',
'<div class="project-config-grid">',
'<div><label for="project-config-workspace-mode">プロジェクトの状態 <span class="field-required">必須</span></label><select id="project-config-workspace-mode" name="workspaceMode"><option value="new" selected>これから新しく作る</option><option value="existing">すでにあるコードを使う</option></select><small class="field-help">通常は「これから新しく作る」のままで構いません。</small></div>',
'<div><label for="project-config-name">プロジェクト名 <span class="field-required">必須</span></label><input id="project-config-name" name="name" maxlength="64" pattern="[a-z0-9][a-z0-9-]{0,63}" placeholder="wordpress-demo" required><small class="field-help">英小文字・数字・ハイフンで入力します。保存後、「新しいジョブ」のプロジェクト選択欄に表示されます。</small></div>',
'<div><label for="project-config-workspace">既存コードがあるフォルダ <span class="field-optional">既存の場合だけ</span></label><input id="project-config-workspace" name="workspace" maxlength="500" placeholder="/home/ubuntu/example-project"><small class="field-help">「すでにあるコードを使う」を選んだ場合だけ入力します。親フォルダや本番の配置先ではなく、プロジェクト直下の絶対パスです。</small></div>',
'</div>',
'<p class="settings-intro"><strong>新規プロジェクトの保存先は自動で決まります。</strong> 例えばプロジェクト名が <code>wordpress-demo</code> なら、<code>/home/ubuntu/chatgpt-projects/wordpress-demo</code> を作成します。</p>',
'<p class="helper"><strong>新規なら「プロジェクトの状態」と「プロジェクト名」だけで登録できます。</strong> 保存しても作業は始まらず、「新しいジョブ」の実行先に追加されます。</p>',
'</section>',
'<details class="advanced-config">',
'<summary><span><strong>詳細設定</strong><small>GitHub連携や本番への自動反映を使う場合だけ開く</small></span></summary>',
'<div class="advanced-config-content">',
'<section class="config-section">',
'<div class="config-section-head"><h3>実行方式</h3><p>通常は「Ubuntuホスト直接実行」のまま変更しません。</p></div>',
'<div class="project-config-grid">',
'<div><label for="project-config-mode">実行方式 <span class="field-optional">通常は変更不要</span></label><select id="project-config-mode" name="executionMode">' +
'<option value="local" selected>Ubuntuホスト直接実行（標準）</option>' +
'<option value="github_direct">GitHub直接編集</option>' +
'<option value="verify_only">検証のみ</option>' +
'</select><small class="field-help">GitHub直接編集は、既存リポジトリへブランチとPRを作る運用が必要な場合だけ選びます。</small></div>',
'</div>',
'<div class="execution-mode-guide" aria-label="実行方式の違い">',
'<div><strong>Ubuntuホスト直接実行</strong>Ubuntu上の専用worktreeで編集・検証します。</div>',
'<div><strong>GitHub直接編集</strong>GitHub上で専用ブランチとPRを作成します。</div>',
'<div><strong>検証のみ</strong>ファイルを変更せず、指定された確認だけを実行します。</div>',
'</div>',
'</section>',
'<section class="config-section">',
'<div class="config-section-head"><h3>本番へ自動反映する設定</h3><p>すでに動いている本番環境と、既存の反映コマンドがある場合だけ設定します。コマンドを新しく考えて入力する欄ではありません。</p></div>',
'<div class="execution-mode-guide" aria-label="本番反映の流れ">',
'<div><strong>1. 作業と検証</strong>専用の作業フォルダでコードを変更し、テストを実行します。</div>',
'<div><strong>2. 本番へ反映</strong>テスト成功後に「本番反映コマンド」を実行します。</div>',
'<div><strong>3. 反映を確認</strong>確認コマンドと稼働確認URLで、本番が正常か確認します。</div>',
'</div>',
'<div class="project-config-grid">',
'<div><label for="project-config-production">本番の配置先 <span class="field-optional">任意</span></label><input id="project-config-production" name="productionRoot" maxlength="500" placeholder="/opt/pseudo-codex-console"><small class="field-help">利用者が実際に使っている本番ファイルのUbuntu上の絶対パスです。作業フォルダとは別です。</small></div>',
...
'<div><label for="project-config-deploy">本番反映コマンド <span class="field-optional">任意</span></label><input id="project-config-deploy" name="deployCommand" maxlength="500" placeholder="/usr/local/libexec/pseudo-codex-deploy-request-console"><small class="field-help">テスト成功後、作業結果を本番へコピー・更新・再起動する既存コマンドです。既存コマンドが分からなければ空欄にします。</small></div>',
...
'<p class="helper"><strong>既存の反映コマンドが分からない場合は、すべて空欄・チェックなしが正解です。</strong> その状態でもコード変更とテストは実行できます。</p>',
'</section>',
'<section class="config-section">',
'<div class="config-section-head"><h3>GitHub / Git 運用</h3><p>実行方式で「GitHub直接編集」を選んだ場合だけ設定します。通常のUbuntuホスト直接実行では不要です。</p></div>',
...
'<div class="project-config-grid">',
'<div><label for="project-config-repository">GitHub Repository <span class="field-optional">Git使用時</span></label><input id="project-config-repository" name="gitRepository" maxlength="300" placeholder="owner/repository"><small class="field-help">既存リポジトリの「所有者名/リポジトリ名」を入力します。URL・メールアドレス・パスワードは入力しません。</small></div>',
'<div><label for="project-config-remote">Git Remote <span class="field-optional">Git使用時</span></label><input id="project-config-remote" name="gitRemote" maxlength="200" value="origin"><small class="field-help">Workspaceですでに設定済みのremote名です。通常はoriginのままです。</small></div>',
'<div><label for="project-config-base">Base Branch <span class="field-optional">Git使用時</span></label><input id="project-config-base" name="gitBaseBranch" maxlength="200" value="main"><small class="field-help">作業を開始する基準ブランチです。</small></div>',
'<div><label for="project-config-prefix">Branch Prefix <span class="field-optional">Git使用時</span></label><input id="project-config-prefix" name="gitBranchPrefix" maxlength="200" value="chatgpt-job"><small class="field-help">ジョブごとの作業ブランチ名の先頭に使います。</small></div>',
'</div>',
'<p class="helper">pushにはUbuntu側ですでに設定済みのGit authorとGitHub認証を使用します。この画面から別アカウントへログインしたり、認証情報を保存したりはしません。</p>',
'</div>',
'</section>',
'</div>',
'</details>',
'<button type="submit" class="settings-save-button">この内容で追加・更新</button>',
'</form>'
].join("");
}
...
(function () {
  ...
  function revealCreatedJob() {
    ...
    var card = document.getElementById("job-" + createdJobId);
    if (!card) return;
...
created.textContent = "作成 " + formatCreatedAt(job.createdAt);
    ...
    summary.append(
      ...
      createCell("job-updated", job.durationText || "経過時間不明")
    );
...
function renderPage(jobs, message) {
  ...
  return [
    ...
    '.kpis{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:24px}.kpi{position:relative;overflow:hidden;background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:17px 18px 16px;min-height:108px}.kpi::before{content:"";position:absolute;inset:0 auto 0 0;width:4px;background:var(--accent)}.kpi-label{font-size:.72rem;font-weight:750;color:#64748b}.kpi-value{display:block;font-size:1.55rem;line-height:1.1;margin-top:10px}.kpi-note{display:inline-flex;margin-top:9px;padding:5px 9px;border-radius:8px;background:var(--tint);color:var(--accent);font-size:.68rem;font-weight:700}',
    ...
    '@keyframes created-job-highlight{0%,70%{background:#fef3c7;box-shadow:inset 4px 0 #f59e0b}100%{background:#fff;box-shadow:inset 4px 0 transparent}}.job-table-header{display:grid;grid-template-columns:minmax(220px,2fr) minmax(110px,1fr) 118px 135px 92px;gap:12px;padding:12px 20px;background:#f8fafc;border-top:1px solid #eef2f7;border-bottom:1px solid #e2e8f0;color:#64748b;font-size:.62rem;font-weight:800;letter-spacing:.04em}.jobs{min-height:320px}.jobs>.job{margin:0;border:0;border-bottom:1px solid #e2e8f0;border-radius:0;background:#fff;padding:0;box-shadow:none;cursor:pointer;transition:background .15s ease}.jobs>.job:hover{background:#f8fafc}.jobs>.job.is-selected{background:#eff6ff}.jobs>.job.is-created{animation:created-job-highlight 6s ease-out both}.jobs>.job:last-child{border-bottom:0}.jobs>.job>:not(.job-row-summary){display:none}.job-row-summary{display:grid;grid-template-columns:minmax(220px,2fr) minmax(110px,1fr) 118px 135px 92px;gap:12px;align-items:center;min-height:88px;padding:13px 20px;font-size:.72rem}.job-identity{min-width:0}.job-identity strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:.8rem;color:#0f172a}.job-identity small{display:block;margin-top:4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:.64rem}.job-created{color:#475569;font-weight:700}.job-id{color:#94a3b8}.job-project,.job-worker,.job-updated{color:#475569;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.job-updated{color:#64748b}.job-state{justify-self:start;border-radius:999px;padding:5px 9px;font-size:.64rem;font-weight:750;white-space:nowrap}',
    '.stage-queued{background:#fef3c7;color:#92400e}.stage-sending_to_chatgpt,.stage-waiting_chatgpt{background:#dcfce7;color:#166534}.stage-executing_command,.stage-writing_file,.stage-verifying{background:#dbeafe;color:#1d4ed8}.stage-completed{background:#e6eee8;color:#3f684c}.stage-failed{background:#dc2626;color:#fff;border:1px solid #991b1b;box-shadow:0 0 0 2px #fee2e2;font-weight:850}.stage-stopped{background:#e2e8f0;color:#475569}.stage-blocked{background:#fef3c7;color:#92400e}',
    ...
    '.visually-hidden{position:absolute!important;width:1px!important;height:1px!important;padding:0!important;margin:-1px!important;overflow:hidden!important;clip:rect(0,0,0,0)!important;white-space:nowrap!important;border:0!important}.detail-job{--job-detail-content-height:clamp(22rem,52dvh,31rem);padding:18px}.detail-job .facts-explained{grid-template-columns:repeat(2,minmax(0,1fr))}.detail-job .facts-explained small{display:block;margin-top:5px;color:#64748b;font-size:.64rem;line-height:1.45}.detail-job>details>pre,.detail-job>details>.chat-thread{display:block;box-sizing:border-box;max-block-size:var(--job-detail-content-height);overflow:auto;overscroll-behavior:contain}.detail-job>details>.history-log{max-block-size:var(--job-detail-content-height);min-block-size:0}.detail-job .conversation-details{border:1px solid #8faac2;box-shadow:0 8px 24px rgba(30,64,91,.12)}.detail-job .conversation-details>summary{display:flex;align-items:center;gap:9px;min-height:58px;padding:10px 12px;background:#f7fbff;color:#17324a}.detail-job .conversation-details>summary::after{margin-left:4px}.chat-summary-icon{display:grid;place-items:center;flex:0 0 30px;width:30px;height:30px;border-radius:50%;background:#06c755;color:#fff;font-size:.65rem;box-shadow:0 0 0 4px #e6f8ed}.chat-summary-copy{display:flex;flex:1;min-width:0;flex-direction:column}.chat-summary-copy strong{font-size:.78rem}.chat-summary-copy small{margin-top:2px;color:#5d7183;font-size:.61rem;font-weight:600}.chat-turn-count{flex:0 0 auto;border-radius:999px;padding:4px 7px;background:#e4eef7;color:#33536e;font-size:.6rem;font-weight:800}.chat-thread{padding:16px 13px 22px;background-color:#b7cbe0;background-image:linear-gradient(135deg,rgba(255,255,255,.08) 25%,transparent 25%,transparent 75%,rgba(255,255,255,.08) 75%);background-size:24px 24px;border-top:1px solid #9fb5ca;scrollbar-gutter:stable}.chat-turn{display:grid;gap:13px;margin-bottom:22px}.chat-turn:last-child{margin-bottom:0}.chat-turn-label{justify-self:center;border-radius:999px;padding:4px 10px;background:rgba(34,57,78,.55);color:#fff;font-size:.6rem;font-weight:800;letter-spacing:.02em}.chat-row{display:flex;align-items:flex-start;gap:8px}.chat-row-user{justify-content:flex-end}.chat-row-assistant{justify-content:flex-start}.chat-avatar{display:grid;place-items:center;flex:0 0 30px;width:30px;height:30px;margin-top:19px;border-radius:50%;background:#fff;color:#35536e;border:1px solid rgba(53,83,110,.2);box-shadow:0 1px 3px rgba(15,23,42,.16);font-size:.62rem;font-weight:850}.chat-row-user .chat-avatar{background:#06c755;color:#fff;border-color:#05a947}.chat-message{display:flex;flex-direction:column;align-items:flex-start;max-width:78%;min-width:0}.chat-row-user .chat-message{align-items:flex-end}.chat-speaker{margin:0 8px 5px;color:#29465f;font-size:.65rem;font-weight:850}.chat-speaker span{margin-left:4px;color:#526b80;font-size:.58rem;font-weight:650}.chat-bubble{position:relative;min-width:48px;border-radius:17px;padding:10px 12px;background:#fff;box-shadow:0 1px 3px rgba(15,23,42,.16)}.chat-row-user .chat-bubble{background:#91e85f;border-top-right-radius:5px}.chat-row-assistant .chat-bubble{border-top-left-radius:5px}.chat-row-user .chat-bubble::after{content:"";position:absolute;top:0;right:-7px;border:0 solid transparent;border-top-width:11px;border-left:9px solid #91e85f}.chat-row-assistant .chat-bubble::before{content:"";position:absolute;top:0;left:-7px;border:0 solid transparent;border-top-width:11px;border-right:9px solid #fff}.chat-bubble pre,.chat-meta pre{border:0!important;background:transparent!important;color:#102235!important;padding:0!important;font:500 .75rem/1.65 Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif!important;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;tab-size:2}.chat-message time{margin:4px 8px 0;color:#36546d;font-size:.56rem;font-weight:650}.chat-meta{align-self:stretch;margin-top:8px!important;border-color:rgba(51,83,110,.3)!important;background:rgba(247,251,255,.86)!important}.chat-meta summary{display:flex;justify-content:space-between;gap:8px;padding:8px 10px!important;font-size:.63rem!important;color:#29465f}.chat-meta summary small{color:#60778b;font-size:.58rem}.chat-meta pre{max-height:14rem;overflow:auto;padding:9px 10px!important;border-top:1px solid rgba(51,83,110,.18)!important;font-family:ui-monospace,SFMono-Regular,Consolas,monospace!important;font-size:.66rem!important}',
    '[data-conversation-style="slack"]{border-color:#d8d8d8!important;box-shadow:0 2px 8px rgba(29,28,29,.08)!important}[data-conversation-style="slack"]>summary{background:#f8f8f8!important;color:#1d1c1d!important}[data-conversation-style="slack"] .chat-summary-icon{border-radius:8px;background:#4a154b;box-shadow:none;color:#fff;font-size:1rem}[data-conversation-style="slack"] .chat-summary-copy small{color:#616061}[data-conversation-style="slack"] .chat-turn-count{background:#f0e8f1;color:#4a154b}[data-conversation-style="slack"] .chat-thread{padding:12px 8px 18px;background:#fff;background-image:none;border-top-color:#d8d8d8}[data-conversation-style="slack"] .chat-turn{gap:6px;margin-bottom:14px}[data-conversation-style="slack"] .chat-turn-label{justify-self:start;margin-left:48px;border:1px solid #ddd;border-radius:999px;padding:3px 8px;background:#fff;color:#616061;font-weight:700}[data-conversation-style="slack"] .chat-row,[data-conversation-style="slack"] .chat-row-user,[data-conversation-style="slack"] .chat-row-assistant{justify-content:flex-start;gap:10px;border-radius:6px;padding:7px 8px}[data-conversation-style="slack"] .chat-row-user{background:#faf5ff;border-left:4px solid #7c3aed}[data-conversation-style="slack"] .chat-row-assistant{background:#eff6ff;border-left:4px solid #1264a3}[data-conversation-style="slack"] .chat-row-user:hover{background:#f3e8ff}[data-conversation-style="slack"] .chat-row-assistant:hover{background:#dbeafe}[data-conversation-style="slack"] .chat-row-user .chat-speaker{color:#581c87}[data-conversation-style="slack"] .chat-row-assistant .chat-speaker{color:#0c4a6e}[data-conversation-style="slack"] .chat-avatar{flex-basis:36px;width:36px;height:36px;margin-top:1px;border:0;border-radius:8px;background:#1264a3;color:#fff;box-shadow:none;font-size:.68rem}[data-conversation-style="slack"] .chat-row-user .chat-avatar{background:#4a154b;border:0}[data-conversation-style="slack"] .chat-message,[data-conversation-style="slack"] .chat-row-user .chat-message{align-items:stretch;flex:1;max-width:none}[data-conversation-style="slack"] .chat-speaker{display:flex;align-items:baseline;gap:6px;margin:0 0 2px;color:#1d1c1d;font-size:.76rem}[data-conversation-style="slack"] .chat-speaker span{margin:0;color:#616061}[data-conversation-style="slack"] .chat-speaker time{margin:0;color:#616061;font-size:.58rem;font-weight:500}[data-conversation-style="slack"] .chat-bubble,[data-conversation-style="slack"] .chat-row-user .chat-bubble,[data-conversation-style="slack"] .chat-row-assistant .chat-bubble{width:100%;border-radius:0;padding:0;background:transparent;box-shadow:none}[data-conversation-style="slack"] .chat-row-user .chat-bubble::after,[data-conversation-style="slack"] .chat-row-assistant .chat-bubble::before{content:none}[data-conversation-style="slack"] .chat-bubble pre{color:#1d1c1d!important;font-size:.75rem!important;line-height:1.5!important}[data-conversation-style="slack"] .chat-meta{border-color:#ddd!important;background:#f8f8f8!important}[data-conversation-style="slack"] .chat-meta summary{color:#1d1c1d}',
    '.queue-summary{display:flex;align-items:center;gap:7px;overflow:auto;padding:9px 20px;border-top:1px solid #eef2f7;background:#fbfcfe;white-space:nowrap}.queue-summary span{display:inline-flex;align-items:center;gap:5px;border:1px solid #e2e8f0;border-radius:999px;padding:4px 8px;background:#fff;color:#475569;font-size:.64rem}.queue-summary strong{color:#1e293b;font-weight:800}.queue-summary span:nth-child(4){border-color:#fecaca;background:#fef2f2;color:#b91c1c}.queue-summary span:nth-child(4) strong{color:#991b1b}.empty{padding:48px 20px;text-align:center;color:#64748b}',
    '.settings-area{margin-top:20px;display:grid;gap:12px}.settings-details{background:#fff;border:1px solid #dbe4ee;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(15,23,42,.04)}.settings-details>summary{display:grid;grid-template-columns:minmax(0,1fr) auto auto;align-items:center;gap:10px;min-height:64px;padding:13px 18px;list-style:none;cursor:pointer}.settings-details>summary::-webkit-details-marker{display:none}.settings-details>summary::after{content:"＋";display:grid;place-items:center;width:28px;height:28px;border-radius:8px;background:#eff6ff;color:#1d4ed8;font-size:1rem;font-weight:800}.settings-details[open]>summary::after{content:"−"}.settings-summary-copy{display:flex;min-width:0;flex-direction:column;gap:3px}.settings-summary-copy strong{color:#0f172a;font-size:.9rem}.settings-summary-copy small{color:#64748b;font-size:.68rem;font-weight:600}.settings-summary-badge{border-radius:999px;padding:5px 8px;background:#f1f5f9;color:#475569;font-size:.61rem;font-weight:800;white-space:nowrap}.settings-details[open]>summary{border-bottom:1px solid #e2e8f0;background:#f8fbff}.settings-content{padding:18px}.settings-intro{margin:0 0 14px;padding:13px 14px;border:1px solid #bfdbfe;border-radius:10px;background:#eff6ff;color:#1e3a5f;font-size:.73rem;line-height:1.65}.settings-intro strong{color:#1d4ed8}.settings-flow{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:0 0 16px;padding:0;list-style:none}.settings-flow li{position:relative;border:1px solid #dbe4ee;border-radius:10px;padding:11px 11px 11px 39px;background:#fff;color:#475569;font-size:.68rem;line-height:1.5}.settings-flow span{position:absolute;left:10px;top:10px;display:grid;place-items:center;width:20px;height:20px;border-radius:50%;background:#2563eb;color:#fff;font-size:.62rem;font-weight:850}.config-section{display:grid;gap:10px;padding-top:4px}.config-section+.config-section{margin-top:4px;padding-top:16px;border-top:1px solid #e2e8f0}.config-section-head h3{margin:0;color:#0f172a;font-size:.82rem}.config-section-head p{margin:4px 0 0;color:#64748b;font-size:.68rem;line-height:1.55}.project-grid{display:grid;gap:12px}.project-card{background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:14px}.project-card-head{display:flex;justify-content:space-between;gap:12px}.project-card h3{margin:0;font-size:.88rem}.project-card a{color:#1d4ed8;font-weight:700}.project-facts{display:flex;gap:10px;flex-wrap:wrap;margin-top:8px;color:#64748b;font-size:.7rem}.project-note{margin-top:9px;padding:9px 10px;border-left:3px solid #16a34a;background:#f0fdf4;color:#166534;font-size:.7rem}.project-config-form{display:grid;gap:16px}.project-config-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.project-config-form label{display:flex;align-items:center;gap:6px;font-size:.7rem;font-weight:750;color:#334155;margin-bottom:5px}.field-required,.field-optional{border-radius:999px;padding:2px 5px;font-size:.55rem;font-weight:800}.field-required{background:#dbeafe;color:#1d4ed8}.field-optional{background:#f1f5f9;color:#64748b}.field-help{display:block;margin-top:4px;color:#64748b;font-size:.61rem;line-height:1.45}.project-config-form input,.project-config-form select{width:100%;border:1px solid #cbd5e1;border-radius:8px;padding:9px 10px;background:#fff}.project-config-form input:focus,.project-config-form select:focus{outline:3px solid #dbeafe;border-color:#2563eb}.prefixed-input{display:flex;align-items:stretch}.prefixed-input span{display:flex;align-items:center;flex:0 0 auto;border:1px solid #cbd5e1;border-right:0;border-radius:8px 0 0 8px;padding:9px 10px;background:#e2e8f0;color:#475569;font-size:.7rem;font-weight:750;user-select:none}.prefixed-input input{min-width:0;border-radius:0 8px 8px 0}.execution-mode-guide{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.execution-mode-guide div{border:1px solid #e2e8f0;border-radius:9px;padding:9px;background:#f8fafc;color:#64748b;font-size:.63rem;line-height:1.45}.execution-mode-guide strong{display:block;margin-bottom:2px;color:#334155;font-size:.67rem}.project-git-box{border:1px solid #dbe4ee;border-radius:10px;padding:13px;background:#f8fafc}.project-git-box h3{margin:0 0 4px;font-size:.78rem}.checkbox-row{display:flex;gap:14px;flex-wrap:wrap}.checkbox-row label{display:inline-flex;align-items:center;gap:7px;margin:0}.checkbox-row input{width:auto}.settings-save-button{justify-self:start;min-height:42px;border:0;border-radius:9px;padding:10px 16px;background:#2563eb;color:#fff;font-weight:800}.helper{font-size:.72rem;color:#64748b}@media(max-width:780px){.settings-details>summary{grid-template-columns:minmax(0,1fr) auto;padding:12px 14px}.settings-summary-badge{display:none}.settings-content{padding:14px}.settings-flow,.execution-mode-guide{grid-template-columns:1fr}}',
...
':root{--shell-bg:#f3f6fb;--nav-muted:#8d98ad;--brand-accent:#6d5dfc;--brand-accent-2:#00b7c7;--panel-border:#dfe5ef;--panel-shadow:0 10px 34px rgba(15,23,42,.07)}body{background:var(--shell-bg);color:#172033}.app-shell{background:linear-gradient(135deg,#f6f8fc 0%,#eef3f9 58%,#f8f5ff 100%)}.sidebar{background:linear-gradient(180deg,#121827 0%,#0d1320 100%);border-right:1px solid rgba(255,255,255,.06);box-shadow:10px 0 32px rgba(15,23,42,.12)}.brand{position:relative;padding-bottom:24px}.brand::after{content:"";position:absolute;left:0;right:0;bottom:8px;height:1px;background:linear-gradient(90deg,rgba(109,93,252,.8),rgba(0,183,199,.25),transparent)}.brand strong{letter-spacing:.13em}.brand span{color:#7f8aa0}.side-nav{gap:5px}.side-nav a{position:relative;border:1px solid transparent;border-radius:10px;color:var(--nav-muted);transition:background .16s ease,border-color .16s ease,color .16s ease,transform .16s ease}.side-nav a:hover{transform:translateX(2px);border-color:rgba(255,255,255,.08);background:rgba(255,255,255,.055);color:#fff}.side-nav a[aria-current="page"]{border-color:rgba(109,93,252,.42);background:linear-gradient(90deg,rgba(109,93,252,.27),rgba(109,93,252,.08));color:#fff;box-shadow:inset 3px 0 0 #8b7cff}.nav-mark{display:grid;place-items:center;width:24px;height:24px;border-radius:7px;background:rgba(255,255,255,.06);font-size:.72rem}.server-status{border-color:rgba(255,255,255,.08);background:rgba(255,255,255,.035)}.online-dot{box-shadow:0 0 0 4px rgba(34,197,94,.12)}.topbar{height:78px;border-bottom:1px solid rgba(203,213,225,.8);background:rgba(255,255,255,.88);backdrop-filter:blur(18px);box-shadow:0 4px 18px rgba(15,23,42,.035)}.topbar-title{display:flex;align-items:center;gap:12px}.topbar-title-mark{display:grid;place-items:center;width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,#6d5dfc,#00a9bd);box-shadow:0 8px 20px rgba(109,93,252,.24);color:#fff;font-size:.82rem;font-weight:900}.topbar h1{font-size:1.18rem;letter-spacing:-.025em}.topbar p{font-size:.7rem}.top-search{min-height:42px;border-color:#d5dce8;border-radius:11px;background:#f7f9fc;box-shadow:inset 0 1px 2px rgba(15,23,42,.025);transition:border-color .16s ease,box-shadow .16s ease,background .16s ease}.top-search:focus{outline:0;border-color:#8b7cff;background:#fff;box-shadow:0 0 0 4px rgba(109,93,252,.12)}.primary-button{min-height:42px;border-radius:11px;background:linear-gradient(135deg,#6655f5,#5143dc);box-shadow:0 8px 18px rgba(81,67,220,.22);transition:transform .16s ease,box-shadow .16s ease,filter .16s ease}.primary-button:hover{transform:translateY(-1px);box-shadow:0 11px 22px rgba(81,67,220,.28);filter:saturate(1.08)}.primary-button:active{transform:translateY(0)}.main{max-width:1800px;width:100%;margin:0 auto;padding-top:26px}.overview-hero{position:relative;display:flex;align-items:flex-end;justify-content:space-between;gap:24px;overflow:hidden;margin-bottom:16px;border:1px solid rgba(109,93,252,.2);border-radius:16px;padding:22px 24px;background:linear-gradient(118deg,#171d31 0%,#24254b 58%,#225d6b 132%);box-shadow:0 16px 38px rgba(26,31,65,.16);color:#fff}.overview-hero::after{content:"";position:absolute;right:-58px;top:-96px;width:250px;height:250px;border:44px solid rgba(255,255,255,.055);border-radius:50%}.overview-hero-copy,.overview-hero-meta{position:relative;z-index:1}.overview-eyebrow{display:block;margin-bottom:7px;color:#9ba7ff;font-size:.62rem;font-weight:850;letter-spacing:.12em;text-transform:uppercase}.overview-hero h2{margin:0;font-size:1.3rem;letter-spacing:-.035em}.overview-hero p{max-width:680px;margin:7px 0 0;color:#bdc7da;font-size:.72rem;line-height:1.6}.overview-hero-meta{display:flex;align-items:center;gap:9px;white-space:nowrap}.overview-live{display:inline-flex;align-items:center;gap:7px;border:1px solid rgba(255,255,255,.14);border-radius:999px;padding:7px 10px;background:rgba(255,255,255,.075);font-size:.62rem;font-weight:800}.overview-live::before{content:"";width:7px;height:7px;border-radius:50%;background:#42e58c;box-shadow:0 0 0 5px rgba(66,229,140,.12)}.overview-refresh{color:#9faac0;font-size:.59rem}.kpis{gap:14px}.kpi{border-color:var(--panel-border);border-radius:15px;background:rgba(255,255,255,.94);box-shadow:0 7px 24px rgba(15,23,42,.045);transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease}.kpi:hover{transform:translateY(-2px);border-color:#cbd4e3;box-shadow:0 12px 28px rgba(15,23,42,.075)}.kpi::before{inset:0 0 auto 0;width:auto;height:3px}.kpi-head{display:flex;align-items:center;justify-content:space-between;gap:10px}.kpi-icon{display:grid;place-items:center;width:30px;height:30px;border-radius:9px;background:var(--tint);color:var(--accent);font-size:.75rem;font-weight:900}.kpi-value{font-size:1.72rem;letter-spacing:-.045em}.queue-card,.detail-panel,.settings-details{border-color:var(--panel-border);border-radius:15px;box-shadow:var(--panel-shadow)}.queue-toolbar{padding:20px 21px;border-bottom:1px solid #e8edf4;background:linear-gradient(180deg,#fff,#fbfcfe)}.queue-toolbar h2{letter-spacing:-.025em}.job-table-header{background:#f6f8fb;border-bottom:1px solid #e4e9f1;color:#78849a;font-size:.6rem;letter-spacing:.06em;text-transform:uppercase}.job-row-summary{border-bottom-color:#edf0f5;transition:background .15s ease,box-shadow .15s ease}.job-row-summary:hover{background:#f6f8ff;box-shadow:inset 3px 0 0 #7c6df7}.job.is-selected .job-row-summary{background:#f0efff;box-shadow:inset 3px 0 0 #6655f5}.job-state{border-radius:999px}.detail-panel{position:sticky;top:96px}.detail-empty{min-height:300px;display:grid;place-content:center;text-align:center;background:radial-gradient(circle at 50% 20%,rgba(109,93,252,.08),transparent 42%)}.new-job-panel{border:1px solid rgba(109,93,252,.28);border-radius:15px;background:#fff;box-shadow:0 18px 42px rgba(33,37,70,.13)}.panel-heading{background:linear-gradient(115deg,#f4f2ff,#f7fbff);border-bottom:1px solid #e3e1f8}.filters select{border-radius:9px;background:#fff}.result-count{border-radius:999px;padding:6px 9px;background:#f1f4f8;color:#596579}.settings-area{gap:14px}.settings-details>summary{background:rgba(255,255,255,.95)}.settings-details>summary:hover.settings-details>summary::after{background:#f0efff;color:#6655f5}.settings-details[open]>summary{background:linear-gradient(90deg,#f7f6ff,#fbfdff)}.settings-intro{border-color:#d8d4ff;background:#f5f3ff;color:#3f3a68}.settings-intro strong{color:#5747d7}.settings-flow span{background:linear-gradient(135deg,#6655f5,#00a9bd)}.footer{background:rgba(255,255,255,.82);backdrop-filter:blur(12px)}@media(max-width:1050px){.overview-hero{align-items:flex-start;flex-direction:column}.overview-hero-meta{align-self:flex-start}}@media(max-width:780px){.topbar-title-mark{display:none}.overview-hero{margin-bottom:12px;padding:18px}.overview-hero h2{font-size:1.08rem}.overview-hero-meta{width:100%;justify-content:space-between}.kpi-icon{width:27px;height:27px}.detail-panel{position:static}.side-nav a[aria-current="page"]{box-shadow:inset 0 -3px 0 #8b7cff}.main{padding-top:16px}}',
    "</style>",
    "</head>",
    "<body>",
    '<div class="app-shell">',
    ...
    "</aside>",
    '<header class="topbar">',
    '<div class="topbar-title"><span class="topbar-title-mark" aria-hidden="true">PC</span><div><h1>Operations overview</h1><p>ChatGPTとUbuntuの処理状況をリアルタイム監視</p></div></div>',
    ...
    '<div class="overview-hero-copy"><span class="overview-eyebrow">Workspace control center</span><h2 id="overview-heading">作業状況を、ひとつの画面で把握</h2><p>依頼の登録から実行、会話、検証、本番反映までを一元管理します。ジョブを選ぶと右側に必要な情報だけを表示します。</p></div>',
    '<div class="overview-hero-meta"><span class="overview-live">Dispatcher online</span><span class="overview-refresh">5秒ごとに更新</span></div>',
    '</section>',
    '<section class="kpis" aria-label="ジョブ状況">',
    '<article class="kpi" style="--accent:#6655f5;--tint:#f0efff"><div class="kpi-head"><span class="kpi-label">実行中</span><span class="kpi-icon" aria-hidden="true">▶</span></div><strong class="kpi-value" data-kpi-value="running">', String(runningCount), '</strong><span class="kpi-note">Workerが処理中</span></article>',
    '<article class="kpi" style="--accent:#d97706;--tint:#fff7df"><div class="kpi-head"><span class="kpi-label">実行待ち</span><span class="kpi-icon" aria-hidden="true">⌛</span></div><strong class="kpi-value" data-kpi-value="queued">', String(queuedCount), '</strong><span class="kpi-note">キューに待機</span></article>',
    '<article class="kpi" style="--accent:#dc3f55;--tint:#fff0f2"><div class="kpi-head"><span class="kpi-label">要対応</span><span class="kpi-icon" aria-hidden="true">!</span></div><strong class="kpi-value" data-kpi-value="attention">', String(attentionCount), '</strong><span class="kpi-note">失敗・保留</span></article>',
    '<article class="kpi" style="--accent:#079c78;--tint:#eafaf5"><div class="kpi-head"><span class="kpi-label">完了</span><span class="kpi-icon" aria-hidden="true">✓</span></div><strong class="kpi-value" data-kpi-value="completed">', String(completedCount), '</strong><span class="kpi-note">保存済み</span></article>',
    "</section>",
    ...
    '<div class="job-table-header" aria-hidden="true"><span>ジョブ</span><span>プロジェクト</span><span>状態</span><span>担当</span><span>経過・所要</span></div>',
...
if (values.createWorkspace) {
fs.mkdirSync(values.config.workspace, { recursive: true });
}
...
await readBody(request)
  ...
  response.writeHead(303, {
    Location: "/?created=" + encodeURIComponent(job.id)
  });
  response.end();
} catch (error) {
  sendJson(response, 400, {
