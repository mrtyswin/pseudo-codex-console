...
'<div class="config-section-head"><h3>1. 最初に入力する3項目</h3><p>この3項目で、どのフォルダに対して何をするかを決めます。保存しても作業は始まらず、「新しいジョブ」で選べる実行先が追加されます。</p></div>',
...
'<div><label for="project-config-mode">実行方式 <span class="field-required">必須</span></label><select id="project-config-mode" name="executionMode">' +
...
'</select><small class="field-help">コードを変更する場所と、完了後の反映方法を決めます。</small></div>',
'<div><label for="project-config-workspace">Ubuntu上の作業フォルダ <span class="field-required">必須</span></label><div class="prefixed-input"><span aria-hidden="true">/home/ubuntu/</span><input id="project-config-workspace" name="workspace" maxlength="487" placeholder="chatgpt-projects/wordpress-demo" required></div><small class="field-help">左側は固定です。/home/ubuntu/ より後ろだけ入力します。絶対パスを貼り付けても保存できます。</small></div>',
'</div>',
...
'</div>',
'<p class="helper">保存後は、画面上部の「新しいジョブ」でこのプロジェクトを選んで作業を依頼します。このフォームからジョブ自体は登録されません。</p>',
'</section>',
'<section class="config-section">',
'<div class="config-section-head"><h3>2. 本番へ反映する設定</h3><p>ジョブで変更・検証した内容を、実際に利用中のWebアプリやサービスへ自動反映する場合だけ設定します。設定しなければ、作業フォルダでの変更と検証までで終了します。</p></div>',
