...
function renderPage(jobs, message) {
  ...
  return [
    ...
    '.new-job-panel{background:#fff;border:1px solid #bfdbfe;border-radius:14px;padding:20px;margin-bottom:20px;box-shadow:0 18px 45px rgba(15,23,42,.08)}.new-job-panel[hidden]{display:none}.panel-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;margin-bottom:10px}.panel-heading h2{font-size:1.05rem;margin:0}.panel-heading p{color:#64748b;font-size:.78rem;margin:4px 0 0}.icon-button{border:0;background:transparent;color:#64748b;font-size:1.4rem;cursor:pointer;padding:0 6px}.new-job-form{display:grid;grid-template-columns:minmax(180px,.7fr) minmax(220px,1.3fr) auto;gap:12px;align-items:end}.field label{display:block;font-size:.72rem;font-weight:750;color:#475569;margin-bottom:6px}.field-help{display:block;color:#64748b;font-size:.68rem;line-height:1.45;margin:6px 1px 0}.field input,.field textarea,.field select{width:100%;border:1px solid #cbd5e1;border-radius:9px;padding:10px 11px;background:#fff;color:#0f172a}.field textarea{min-height:108px;max-height:260px;resize:vertical}.submit-field button{width:100%}',
    ...
    '.stage-queued{background:#fef3c7;color:#92400e}.stage-sending_to_chatgpt,.stage-waiting_chatgpt{background:#dcfce7;color:#166534}.stage-executing_command,.stage-writing_file,.stage-verifying{background:#dbeafe;color:#1d4ed8}.stage-completed{background:#dcfce7;color:#166534}.stage-failed{background:#dc2626;color:#fff;border:1px solid #991b1b;box-shadow:0 0 0 2px #fee2e2;font-weight:850}.stage-stopped{background:#e2e8f0;color:#475569}.stage-blocked{background:#fef3c7;color:#92400e}',
    ...
    '<div class="panel-heading"><div><h2>新規ジョブ</h2><p>実行先を選び、短いタイトルと具体的な指示を入力してください。</p></div><button type="button" class="icon-button" data-new-job-close aria-label="閉じる">×</button></div>',
    '<form method="post" action="/jobs" class="new-job-form">',
    '<div class="field"><label for="project">実行するプロジェクト</label><select id="project" name="project" required>', renderProjectOptions(), '</select><small class="field-help">変更対象のプロジェクトを選択します。</small></div>',
    '<div class="field"><label for="title">ジョブタイトル</label><input id="title" name="title" maxlength="200" placeholder="例：新規ジョブ登録フォームの説明を改善" required><small class="field-help">一覧で内容を判別できる短い名前を入力します。</small></div>',
    '<div class="field" style="grid-column:1/-1"><label for="instruction">実行してほしい内容</label><textarea id="instruction" name="instruction" maxlength="20000" placeholder="例：新規ジョブ登録フォームの各項目に、初めて使う人でも入力内容が分かる説明文と具体例を追加してください。既存の登録処理は変更しないでください。" required></textarea><small class="field-help">目的、変更箇所、守る条件、完了と判断できる状態を具体的に書くと正確に実行できます。</small></div>',
    '<div class="submit-field" style="grid-column:1/-1"><button type="submit" class="primary-button">この内容で登録</button></div>',
    "</form>",
    "</section>",
