...
assert.ok(page.includes('<strong>実行中</strong>'));
assert.ok(page.includes('<strong>待機中</strong>'));
assert.ok(page.includes('<strong>完了</strong>'));
assert.ok(page.includes('<strong>失敗</strong>'));
assert.ok(page.includes('<strong>停止</strong>'));
assert.ok(clientScript.includes("var completed = jobs.filter(function (job) { return job.stage === 'completed'; }).length;"));
